...
function addMessage(sender, text) {
  ...
  function typeEffect() {
    if (index < text.length) {
      ...
      index++;
      setTimeout(typeEffect, 25); // controls typing speed
...
function getAnswer(userText) {
  ...
  const match = qaDataset.find(item => input.includes(item.q));
  return match ? match.a : "I'm sorry, I don't have information about that yet. Please ask something else related to DevBay.";
}
