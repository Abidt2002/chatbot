...
function getAnswer(userText) {
  const input = cleanText(userText);
  let bestMatch = null;
  let highestScore = 0;

  for (const pair of qaDataset) {
    const question = cleanText(pair.q);
    let score = 0;
    if (input.includes(question) || question.includes(input)) {
      score += 2;
    }
    // Simple fuzzy score: common words count
    const common = question.split(" ").filter(w => input.includes(w)).length;
    score += common;
  ...
  if (bestMatch && highestScore > 1) return bestMatch.a;
  return "I'm sorry, I don’t have information about that yet. Please ask something related to DevBay’s services or projects.";
}
